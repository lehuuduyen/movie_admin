...
( function( window, document, $, app, undefined ) {
	...
	app.moveRowToAttached = function( $li ) {
		...
		var fieldName = this.search.fieldName;
		
		
		if(fieldName) {
			fieldName = $li.parent().parent().parent().attr("data-fieldname")
			var $wrap  = $( '[data-fieldname="' + fieldName + '"]' );
			if(app.iterator) {
				if(document.getElementById("cmb-group-post_table_of_content_group-0")){
					var $wrap = $("#cmb-group-post_table_of_content_group-" + app.iterator).find('[data-fieldname="' + fieldName + '"]');
				}
				if(fieldName.indexOf("page_dog_ranking_group") > 0){
					var $wrap = $("#cmb-group-page_dog_ranking_group-" + app.iterator).find('[data-fieldname="' + fieldName + '"]');
				}
				if(fieldName.indexOf("page_dog_size_group") > 0){
					var $wrap = $("#cmb-group-page_dog_size_group-" + app.iterator).find('[data-fieldname="' + fieldName + '"]');
				}
				if(fieldName.indexOf("page_dog_group_group") > 0){
					var $wrap = $("#cmb-group-page_dog_group_group-" + app.iterator).find('[data-fieldname="' + fieldName + '"]');
				}
			}
	...
	app.$retrievedPosts = function() {
		...
		var fieldName = this.search.fieldName;
		if(fieldName) {
			var $wrap  = $( '[data-fieldname="' + fieldName + '"] .retrieved-wrap' );
			if(app.iterator) {
				
				if(document.getElementById("cmb-group-post_table_of_content_group-0")){
					var $wrap = $("#cmb-group-post_table_of_content_group-" + app.iterator).find('[data-fieldname="' + fieldName + '"] .retrieved-wrap');
				}
				if(fieldName.indexOf("page_dog_ranking_group") > 0){
					var $wrap = $("#cmb-group-page_dog_ranking_group-" + app.iterator).find('[data-fieldname="' + fieldName + '"] .retrieved-wrap');
				}
				if(fieldName.indexOf("page_dog_size_group") > 0){
					var $wrap = $("#cmb-group-page_dog_size_group-" + app.iterator).find('[data-fieldname="' + fieldName + '"] .retrieved-wrap');
				}
				if(fieldName.indexOf("page_dog_group_group") > 0){
					var $wrap = $("#cmb-group-page_dog_group_group-" + app.iterator).find('[data-fieldname="' + fieldName + '"] .retrieved-wrap');
				}
			}
		} else {
			var $wrap  = app.$.retrievedPosts;
		}
	...
	app.SearchView = window.Backbone.View.extend({
		...
		send: function() {
			...
			if(document.querySelector('input[name="search_posts_internal_type"]:checked')){
				data.typeSearch = document.querySelector('input[name="search_posts_internal_type"]:checked').value
			}
	...
	app.openSearch = function( $button ) {
		...
		//Custom wanqol
		app.iterator = $button.parents().eq(6).attr('data-iterator');
