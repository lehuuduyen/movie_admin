    ...
    cmb_tags = {
        ...
        quickClicks : function(el) {
            ...
            $.each( current_tags, function( key, val ) {
                ...
                li = $('<li style="margin-left:24px"  />').text( val );
    
                // If tags editing isn't disabled, create the X button.
                if ( ! disabled ) {
                    xbutton = $( '<button style="margin-top:-2px" type="button" id="' + id + '-check-num-' + key + '"  class="ntdelbutton"><span class="remove-tag-icon" aria-hidden="true"></span><span class="screen-reader-text">Remove term: asads</span></button>' );
                    xbutton.click( function(){ cmb_tags.parseTags(this); });
                    li.prepend('&nbsp;').prepend( xbutton );
                }
    
                // Append the li to the tag list.
