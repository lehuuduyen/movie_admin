(window.CMB2 = window.CMB2 || {}),
    (function (window, document, $, cmb, undefined) {
        "use strict";
        var $document,
            l10n = window.cmb2_l10,
            setTimeout = window.setTimeout,
            $id = function (selector) {
                return $(document.getElementById(selector));
            };
        cmb.$id = $id;
        // duyen cmb-type-custom-attached-posts
        var defaults = {
            idNumber: !1,
            repeatEls: 'input:not([type="button"],[id^=filelist]),select,textarea,.cmb2-media-status,.cmb-type-custom-attached-posts',
            ...
            styleBreakPoint: 450,
            mediaHandlers: {},
            defaults: { time_picker: l10n.defaults.time_picker, date_picker: l10n.defaults.date_picker, color_picker: l10n.defaults.color_picker || {}, code_editor: l10n.defaults.code_editor },
            media: { frames: {} },
        };
        (cmb.init = function () {
            ($document = $(document)), $.extend(cmb, defaults), cmb.trigger("cmb_pre_init");
            var $metabox = cmb.metabox(),
                $repeatGroup = $metabox.find(".cmb-repeatable-group");
            cmb.initPickers($metabox.find('input[type="text"].cmb2-timepicker'), $metabox.find('input[type="text"].cmb2-datepicker'), $metabox.find('input[type="text"].cmb2-colorpicker')),
                ...
                $id("addtag").length && cmb.listenTagAdd(),
                ...
                wp.data &&
                    ...
                    wp.data.subscribe(function () {
                        ...
                        if (editor && editor.isSavingPost && editor.isSavingPost() && window.tinyMCE.editors.length)
                            for (var i = 0; i < window.tinyMCE.editors.length; i++) window.tinyMCE.activeEditor !== window.tinyMCE.editors[i] && window.tinyMCE.editors[i].save();
                    });
            }),
            ...
            (cmb.resetTitlesAndIterator = function (evt) {
                evt.group &&
                    $(".cmb-repeatable-group.repeatable").each(function () {
                        var $table = $(this),
                            groupTitle = $table.find(".cmb-add-group-row").data("grouptitle");
                        $table.find(".cmb-repeatable-grouping").each(function (rowindex) {
                            var $row = $(this),
                                $rowTitle = $row.find("h3.cmb-group-title");
            ...
            (cmb.toggleCheckBoxes = function (evt) {
                evt.preventDefault();
                var $this = $(this),
                    $multicheck = $this.closest(".cmb-td").find("input[type=checkbox]:not([disabled])");
            ...
            (cmb.handleMedia = function (evt) {
                evt.preventDefault();
                var $el = $(this);
                (cmb.attach_id = !$el.hasClass("cmb2-upload-list") && $el.closest(".cmb-td").find(".cmb2-upload-file-id").val()),
                    (cmb.attach_id = "0" !== cmb.attach_id && cmb.attach_id),
                    ...
                    (media.frames[id] = wp.media({
                        ...
                        (handlers.list = function (selection, returnIt) {
                            var attachmentHtml,
                                fileGroup = [];
                            if (
                                (handlers.list.templates || (handlers.list.templates = { image: wp.template("cmb2-list-image"), file: wp.template("cmb2-list-file") }),
                        ...
                        (handlers.single = function (selection) {
                            handlers.single.templates || (handlers.single.templates = { image: wp.template("cmb2-single-image"), file: wp.template("cmb2-single-file") });
                        ...
                        (handlers.prepareData = function (data, image) {
                            return (
                                image && handlers.getImageData.call(data, 50),
                                (data = data.toJSON()),
                        ...
                        (handlers.getImageData = function (fallbackSize) {
                            var previewW = media.previewSize[0] || fallbackSize,
                                previewH = media.previewSize[1] || fallbackSize,
                                url = this.get("url"),
                            ...
                            return (
                                sizes &&
                                    (sizes[media.sizeName]
                                        ? ((url = sizes[media.sizeName].url), (width = sizes[media.sizeName].width), (height = sizes[media.sizeName].height))
                                        : sizes.large && ((url = sizes.large.url), (width = sizes.large.width), (height = sizes.large.height))),
                                width > previewW && ((height = Math.floor((previewW * height) / width)), (width = previewW)),
                                height > previewH && ((width = Math.floor((previewH * width) / height)), (height = previewH)),
                                width || (width = previewW),
                                height || (height = "svg" === this.get("filename").split(".").pop() ? "100%" : previewH),
                        ...
                        media.frames[id].on("select", handlers.selectFile).on("open", handlers.openModal),
            ...
            (cmb.handleRemoveMedia = function (evt) {
                evt.preventDefault();
                var $this = $(this);
                return $this.is(".cmb-attach-list .cmb2-remove-file-button")
                    ? ($this.parents(".cmb2-media-item").remove(), !1)
            ...
            (cmb.cleanRow = function ($row, prevNum, group) {
                ...
                return (
                    ...
                    $elements.find(":selected").removeAttr("selected", !1),
                    $row.find("h3.cmb-group-title").length && $row.find("h3.cmb-group-title").text($row.data("title").replace("{#}", cmb.idNumber + 1)),
            ...
            (cmb.elReplacements = function ($newInput, prevNum, group) {
                var newID,
                    ...
                    oldVal = $newInput.val(),
                    type = $newInput.prop("type"),
                    defVal = cmb.getFieldArg($newInput, "default"),
                    newVal = void 0 !== defVal && !1 !== defVal ? defVal : "",
                    tagName = $newInput.prop("tagName"),
                    checkable = ("radio" === type || "checkbox" === type) && oldVal,
                    attrs = {};
                if (oldFor) attrs = { for: oldFor.replace("_" + prevNum, "_" + cmb.idNumber) };
                else {
                    var newName,
                        oldName = $newInput.attr("name");
                    (oldID = $newInput.attr("id")),
                        group
                            ? ((newName = oldName ? oldName.replace("[" + prevNum + "][", "[" + cmb.idNumber + "][") : ""), (newID = oldID ? oldID.replace("_" + prevNum + "_", "_" + cmb.idNumber + "_") : ""))
                            : ((newName = oldName ? cmb.replaceLast(oldName, "[" + prevNum + "]", "[" + cmb.idNumber + "]") : ""), (newID = oldID ? cmb.replaceLast(oldID, "_" + prevNum, "_" + cmb.idNumber) : "")),
                        (attrs = { id: newID, name: newName });
                }
                if (("TEXTAREA" === tagName && $newInput.html(newVal), "SELECT" === tagName && void 0 !== defVal)) {
                    var $toSelect = $newInput.find('[value="' + defVal + '"]');
                    $toSelect.length && $toSelect.attr("selected", "selected").prop("selected", "selected");
                }
                return (
                    checkable && ($newInput.removeAttr("checked"), void 0 !== defVal && oldVal === defVal && $newInput.attr("checked", "checked").prop("checked", "checked")),
                    !group && $newInput[0].hasAttribute("data-iterator") && (attrs["data-iterator"] = cmb.idNumber),
                    $newInput
                        .removeClass("hasDatepicker")
                        .val(checkable || newVal)
                        .attr(attrs),
                    $newInput
                );
            }),
            (cmb.newRowHousekeeping = function ($row) {
                var $colorPicker = $row.find(".wp-picker-container"),
                    $list = $row.find(".cmb2-media-status");
                return (
                    $colorPicker.length &&
            ...
            (cmb.setDefaults = function (evt, row) {
                $(cmb.noEmpty, row).each(function () {
                    var $el = $(this),
                        defVal = cmb.getFieldArg($el, "default");
            ...
            (cmb.addGroupRow = function (evt) {
                ...
                for (
                    var $row = $oldRow.clone(),
                        nodeName = $row.prop("nodeName") || "div",
                        getRowId = function (id) {
                            return (id = id.split("-")), id.splice(id.length - 1, 1), id.push(cmb.idNumber), id.join("-");
                        };
                    $table.find('.cmb-repeatable-grouping[data-iterator="' + cmb.idNumber + '"]').length > 0;
                ...
                $oldRow.after($newRow), cmb.afterRowInsert($newRow), cmb.triggerElement($table, { type: "cmb2_add_row", group: !0 }, $newRow);
                $('.check-box-pagination').each(function(i, obj) {
                    if(!this.checked) {
                      $(this).parent().parent().parent().find('.pagination_title').parent().parent().addClass('hidden')
                    }else{
                      $(this).parent().parent().parent().find('.pagination_title').parent().parent().removeClass('hidden')
                    }
                  });
            }),
            (cmb.addAjaxRow = function (evt) {
                evt.preventDefault();
                var $this = $(this),
                    $table = $id($this.data("selector")),
                    $row = $table.find(".empty-row"),
                    prevNum = parseInt($row.find("[data-iterator]").data("iterator"), 10);
                cmb.idNumber = parseInt(prevNum, 10) + 1;
                ...
                cmb.newRowHousekeeping($emptyrow).cleanRow($emptyrow, prevNum),
                    ...
                    cmb.triggerElement($table, { type: "cmb2_add_row", group: !1 }, $emptyrow, $row);
            }),
            (cmb.removeGroupRow = function (evt) {
                ...
                if (!confirmation || window.confirm(confirmation)) {
                    ...
                    cmb.triggerElement($table, "cmb2_remove_group_row_start", $this),
                        ...
                        $parent.remove(),
                        cmb.triggerElement($table, { type: "cmb2_remove_row", group: !0 });
                }
            }),
            (cmb.removeAjaxRow = function (evt) {
                ...
                if (!$this.hasClass("button-disabled")) {
                    var $parent = $this.parents(".cmb-row"),
                        $table = $this.parents(".cmb-repeat-table");
                    if ($table.find(".cmb-row").length <= 2) return cmb.resetRow($parent.find(".cmb-add-row-button"), $this);
                    $parent.hasClass("empty-row") && $parent.prev().addClass("empty-row").removeClass("cmb-repeat-row"),
                        $this.parents(".cmb-repeat-table .cmb-row").remove(),
                        cmb.triggerElement($table, { type: "cmb2_remove_row", group: !1 });
                }
            }),
            ...
            (cmb.shiftRows = function (evt) {
                evt.preventDefault();
                var $this = $(this),
                    $from = $this.parents(".cmb-repeatable-grouping"),
                ...
                if ((cmb.triggerElement($this, "cmb2_shift_rows_enter", $this, $from, $goto), $goto.length)) {
                    ...
                    $from.find(cmb.repeatEls).each(function () {
                        ...
                        // duyen cmb-type-custom-attached-posts
                        (val = $element.hasClass("cmb2-media-status")||
                        $element.hasClass("cmb-type-custom-attached-posts")
                            ? $element.html()
                            : "checkbox" === elType || "radio" === elType
                            ? $element.is(":checked")
                        ...
                        $goto.find(cmb.repeatEls).each(function (index) {
                            ...
                            $element.hasClass("cmb-type-custom-attached-posts")) {
                                ...
                                "checkbox" === elType
                                    ? (inputVals[index].$.prop("checked", $element.is(":checked")), $element.prop("checked", inputVals[index].val))
                                    : "radio" === elType
                                    ? ($element.is(":checked") && inputVals[index].$.attr("data-checked", "true"), inputVals[index].$.is(":checked") && $element.attr("data-checked", "true"))
                                    : "select" === $element.prop("tagName")
                                    ? (inputVals[index].$.prop("selected", $element.is(":selected")), $element.prop("selected", inputVals[index].val))
                                    : (inputVals[index].$.val($element.val()), $element.val(inputVals[index].val));
                        }),
                        $from.find("input[data-checked=true]").prop("checked", !0).removeAttr("data-checked"),
                        $goto.find("input[data-checked=true]").prop("checked", !0).removeAttr("data-checked"),
            ...
            (cmb.initPickers = function ($timePickers, $datePickers, $colorPickers) {
                cmb.trigger("cmb_init_pickers", { time: $timePickers, date: $datePickers, color: $colorPickers }),
            ...
            (cmb.addTimePickerClasses = function ($picker) {
                ...
                (func.count = func.count || 0),
                    setTimeout(function () {
                        $picker.find(".ui-priority-secondary").length
                            ? ($picker.find(".ui-priority-secondary").addClass("button-secondary"), $picker.find(".ui-priority-primary").addClass("button-primary"), (func.count = 0))
                            : func.count < 5 && (func.count++, func($picker));
                    }, 10);
            }),
            (cmb.initColorPickers = function ($selector) {
                $selector.length &&
                    ("object" == typeof jQuery.wp && "function" == typeof jQuery.wp.wpColorPicker
                        ? $selector.each(function () {
                              var $this = $(this),
                                  fieldOpts = $this.data("colorpicker") || {};
            ...
            (cmb.codeEditorArgs = function (overrides) {
                var props = ["codemirror", "csslint", "jshint", "htmlhint"],
                    args = $.extend({}, cmb.defaults.code_editor);
                overrides = overrides || {};
            ...
            (cmb.makeRepeatableSortable = function () {
                ...
                $repeatables.length && $repeatables.sortable({ items: ".cmb-repeat-row", cursor: "move", cancel: "input,textarea,button,select,option,.CodeMirror" });
            }),
            ...
            (cmb.resizeoEmbeds = function () {
                cmb.metabox().each(function () {
                    ...
                    cmb.styleBreakPoint > tableW && ((isSmall = !0), (isSmallest = cmb.styleBreakPoint - 62 > tableW)), (tableW = isSmall ? tableW : Math.round(0.82 * $tableWrap.width() * 0.97));
            ...
            (cmb.doAjax = function ($obj) {
                ...
                if (!(oembed_url.length < 6)) {
                    var field_id = $obj.attr("id"),
                        $context = $obj.closest(".cmb-td"),
                        $embed_container = $context.find(".embed-status"),
                    ...
                    cmb.log("oembed_url", oembed_url, field_id),
                        ...
                        $embed_wrap.html(""),
                        setTimeout(function () {
                            $(".cmb2-oembed:focus").val() === oembed_url &&
                                $.ajax({
                                    type: "post",
                                    dataType: "json",
                                    url: l10n.ajaxurl,
                ...
                if ("function" == typeof filterCb) {
                    ...
                    return (
                        $.each(l10n.fields, function (hash, field) {
                            filterCb(field, hash) && fields.push(field);
                        }),
                        fields
                    );
                }
            ...
            (cmb.getField = function (hash) {
                var field = {};
                if ((hash = hash instanceof jQuery ? hash.data("hash") : hash))
                    try {
                        if (l10n.fields[hash]) throw new Error(hash);
                        cmb.getFields(function (field) {
                            if ("function" == typeof hash) {
                                if (hash(field)) throw new Error(field.hash);
                            } else if (field.id && field.id === hash) throw new Error(field.hash);
                        });
                    } catch (e) {
    ...
    (function (window, document, $, cmb, wysiwyg, undefined) {
        ...
        function getGroupData(data) {
            ...
            return (
                (all[groupid] && all[groupid][fieldid]) ||
                    ((all[groupid] = all[groupid] || {}),
                    (all[groupid][fieldid] = {
                        template: wp.template("cmb2-wysiwyg-" + groupid + "-" + fieldid),
                        defaults: { mce: $.extend({}, tinyMCEPreInit.mceInit["cmb2_i_" + groupid + fieldid]), qt: $.extend({}, tinyMCEPreInit.qtInit["cmb2_i_" + groupid + fieldid]) },
                    }),
        ...
        function initOptions(options) {
            var prop,
                newSettings,
                newQTS,
                nameRegex = new RegExp("cmb2_n_" + options.groupid + options.fieldid, "g"),
        ...
        (wysiwyg.initAll = function () {
            ...
            $(".cmb2-wysiwyg-placeholder").each(function () {
                ...
                !0 === initiated &&
                    (void 0 !== window.QTags && window.QTags._buttonsInit(),
                    $(document).on("cmb2_add_row", wysiwyg.addRow).on("cmb2_remove_group_row_start", wysiwyg.destroyRowEditors).on("cmb2_shift_rows_start", wysiwyg.shiftStart).on("cmb2_shift_rows_complete", wysiwyg.shiftComplete));
        }),
            ...
            (wysiwyg.initRow = function ($row, evt) {
                ...
                $row.find(".cmb2-wysiwyg-inner-wrap").each(function () {
                    ($toReplace = $(this)),
                        ...
                        (defVal = void 0 !== defVal && !1 !== defVal ? defVal : ""),
                        ...
                        0 === toBeDestroyed.length ? wysiwyg.init($toReplace, data) : (toBeInitialized.push([$toReplace, data]), window.setTimeout(delayedInit, 100));
                });
            }),
            (wysiwyg.init = function ($toReplace, data, buttonsInit) {
                ...
                $.extend(data, getGroupData(data)),
                    ...
                    mceActive && window.tinyMCE.init(tinyMCEPreInit.mceInit[data.id]),
                    qtActive && window.quicktags(tinyMCEPreInit.qtInit[data.id]),
                    mceActive && $(document.getElementById(data.id)).parents(".wp-editor-wrap").removeClass("html-active").addClass("tmce-active"),
                    !1 !== buttonsInit && void 0 !== window.QTags && window.QTags._buttonsInit();
            }),
            (wysiwyg.destroy = function (id) {
                if (cmb.canTinyMCE()) {
                    var editor = tinyMCE.get(id);
                    null !== editor && void 0 !== editor
                        ? (editor.destroy(), void 0 === tinyMCEPreInit.mceInit[id] && delete tinyMCEPreInit.mceInit[id], void 0 === tinyMCEPreInit.qtInit[id] && delete tinyMCEPreInit.qtInit[id])
                        : -1 === toBeDestroyed.indexOf(id) && (toBeDestroyed.push(id), window.setTimeout(delayedDestroy, 100));
                }
            }),
    ...
    (function (window, document, $, cmb, counter) {
        "use strict";
        if (!wp.utils || !wp.utils.WordCounter) return cmb.log("Cannot find wp.utils!");
        counter.counters = {};
        var counters = counter.counters,
            wpCounter = new wp.utils.WordCounter();
        (counter.updateCounter = function (field_id) {
            if (!counters.hasOwnProperty(field_id)) return null;
            var instance = counters[field_id],
                wysiwyg = instance.editor && !instance.editor.isHidden(),
                text = wysiwyg ? instance.editor.getContent({ format: "raw" }) : cmb.$id(field_id).val().trim(),
                count = wpCounter.count(text, instance.type),
                exceeded = instance.max && count > instance.max,
                val = instance.max ? instance.max - count : count;
            return instance.$el.parents(".cmb2-char-counter-wrap")[exceeded ? "addClass" : "removeClass"]("cmb2-max-exceeded"), instance.$el.val(val).outerWidth(8 * String(val).length + 15 + "px"), count;
        }),
            (counter.instantiate = function ($el) {
                var data = $el.data();
                if (!(data.fieldId in counters)) {
                    var instance = { $el: $el, max: data.max, type: "words" === data.counterType ? "words" : "characters_including_spaces", editor: !1 };
                    (counters[data.fieldId] = instance), counter.updateCounter(data.fieldId);
                }
            }),
            ...
            (counter.initWysiwyg = function (evt, editor) {
                editor.id in counters && ((counters[editor.id].editor = editor), editor.on("nodechange keyup", counter.countWysiwyg));
            }),
            (counter.addRow = function (evt, $row) {
                $row.find(".cmb2-char-counter").each(function () {
                    var $this = $(this),
                        id = $this.attr("id"),
                        field_id = id.replace(/^char-counter-/, "");
                    $this.attr("data-field-id", field_id).data("field-id", field_id), counter.instantiate($this);
                });
            }),
            (counter.cleanCounters = function () {
                var field_id,
                    remove = [];
                for (field_id in counters) document.getElementById(field_id) || remove.push(field_id);
                remove.length &&
            ...
            (counter.countWysiwyg = _.throttle(function (evt) {
                return evt.hasOwnProperty("element") ? counter.updateCounter($(evt.element).data("id")) : evt.hasOwnProperty("currentTarget") ? counter.updateCounter($(evt.currentTarget).data("id")) : void 0;
            })),
