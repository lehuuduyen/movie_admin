...
if( ! class_exists( 'MAG_CMB2_Field_Post_Search_Ajax' ) ) {
	...
	class MAG_CMB2_Field_Post_Search_Ajax {
		...
		public function render( $field, $value, $object_id, $object_type, $field_type ) {	
			...
			if($field->args( 'limit' ) > 1){
				...
				else if (isset($query_args['isSearch'])) {
					$field_value = '';
				} 
				else {
		...
		public function cmb_post_search_ajax_get_results() {
			...
			if ( ! wp_verify_nonce( $nonce, 'mag_cmb_post_search_ajax_get_results' ) ) {
				...
				if (isset($args['isSearch'])) {
					unset($args['isSearch']);
					$args['post_permalink_search'] = ['slug' => $_POST['query']];
					$isSearch = true;
				...
				if( $_POST['object'] == 'user' ){
					...
					if ( $results->have_posts() ) :
						while ( $results->have_posts() ) : $results->the_post();
							// Define filter "mag_cmb_post_search_ajax_result" to allow customize ajax results.
							$datas[] = apply_filters( 'mag_cmb_post_search_ajax_result', array(
								'value' => isset($isSearch) ? get_post_field('post_name', get_the_ID()) : get_the_title(),
								'data'	=> get_the_ID(),
	...
	{
		if( $params = $q->get( 'post_permalink_search' ) )
    {
			add_filter( 'posts_search', function($q) use ($params)
      {
				global $wpdb;
				$search = '';
				$value = '%' . $wpdb->esc_like( $params['slug'] ) . '%';
				$search .= '(';
        $search .=  $wpdb->prepare( "{$wpdb->posts}.post_name LIKE '%s'", $value);
        $search .= ')';
				if ( ! empty( $search ) ) {
					$search = " AND ({$search}) ";
				}
